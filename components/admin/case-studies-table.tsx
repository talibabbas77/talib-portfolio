...
export function CaseStudiesTable({ items }: { items: CaseStudyRow[] }) {
  ...
  const deleteIds = async (targetIds: string[]) => {
    ...
    try {
      ...
      toast.success(
        targetIds.length === 1
          ? "Case study deleted."
          : `${targetIds.length} case studies deleted.`
      );
      selection.clear();
      setPending(null);
      router.refresh();
  ...
  return (
    <div className="space-y-4">
        ...
        {items.length === 0 ? (
          <div className="p-8">
            <AdminEmptyState
              ...
              body="Create a case study to feature on the public site."
            />
          </div>
        ) : null}
      </AdminTable>
      ...
      <ConfirmModal
        ...
        title={
          pending?.type === "bulk"
            ? `Delete ${pending.ids.length} case studies?`
            : "Delete this case study?"
        }
        description={
          pending?.type === "bulk"
            ? `This permanently removes ${pending.ids.length} selected case studies. This cannot be undone.`
